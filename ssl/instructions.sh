...
openssl genrsa -passin pass:1111 -new -key server.key -out server.csr -subj "/CN=${SERVER_CN}"

# Step 4: sign the certificate with the CA we created it is called self-signing (server.crt)
openssl x509 -req -passin:1111 -days 365 -in server.csr -CA ca.crt -CAkey ca.key -set_serial 01 -out server.crt
